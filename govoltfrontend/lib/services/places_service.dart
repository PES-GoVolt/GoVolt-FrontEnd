...
class PlacesService {
  ...
  Future<List<PlaceSearch>> getAutoComplete(
    ...
    const region = "es";
    Uri url = Uri.parse(
        'https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$search&key=$apiKey&location=$lat%2C$lng&radius=5000&origin=$lat%2C$lng&region=$region&types=(cities)');
  ...
  Future<List<PlaceSearch>> getAutoCompleteCities(
      String search) async {
    if (apiKey == null) await loadJsonData();
    const region = "es";
    Uri url = Uri.parse(
        'https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$search&key=$apiKey&region=$region');
    var response = await http.get(url);
    var json = convert.jsonDecode(response.body);
    var jsonResults = json['predictions'] as List;
    return jsonResults.map((place) => PlaceSearch.fromJson(place)).toList();
