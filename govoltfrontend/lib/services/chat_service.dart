...
class ChatService {
  ...
  static final message = MessageVolt(
      userid: "", content: "", timestamp: "", roomName: "", chatId: "");
  ...
  Future<List<String>> getAllListeners() async {
    ...
    if (chats != null) {
      ...
      roomNames =
          chatsList.map((chat) => chat['room_name'].toString()).toList();
    }
    ...
    messagesRefSingle.onChildAdded.listen((event) async {
      if (event.snapshot.value is Map) {
        String messageReceived =
            "Nuevo mensaje_Alguien quiere unirse a tu ruta";
        _messageArrivedNotificationController.add(messageReceived);
      }
    });
  }
